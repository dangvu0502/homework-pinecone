...
export class ChatService {
    ...
    try {
      ...
      if (this.enablePinecone && this.embeddingService && this.vectorStore) {
        ...
        relevantChunks = searchResults
          .filter(result => typeof result === 'object' && result !== null && 'score' in result && typeof result.score === 'number' && result.score > 0.3) // Lower threshold for better recall
          .map(result => {
            const typedResult = result as { score: number; metadata?: Record<string, unknown> };
            const metadata = typedResult.metadata || {};
            return {
              documentId: String(metadata?.documentId || ''),
              filename: String(metadata?.filename || ''),
              text: String(metadata?.text || ''),
              relevanceScore: typedResult.score || 0,
  ...
  private buildRAGPrompt(query: string, chunks: RetrievedChunk[]): Array<{ role: 'system' | 'user' | 'assistant'; content: string }> {
    if (chunks.length === 0) {
